...
@section('content')
    <div class="container mx-auto mt-10">
        <div class="grid grid-cols-2 mb-3">
            <h1 style="font-size: 30px; font-weight: bold;">Edit Laporan</h1>
        </div>
                ...
                <form action="{{ route('laporans.update', $laporan->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <input
                            ...
                            value="{{ old('tanggal', $laporan->tanggal) }}" name="tanggal" type="date">
                        @error('tanggal')
                    ...
                    <div class="mb-4">
                        ...
                        <input
                            ...
                            value="{{ old('lokasi', $laporan->lokasi) }}" name="lokasi" type="text">
                        @error('lokasi')
                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="mb-4">
                        ...
                        <select
                            ...
                            name="jenis_sampah">
                            <option value="Organik" {{ old('jenis_sampah', $laporan->jenis_sampah) == 'Organik' ? 'selected' : '' }}>Organik</option>
                            <option value="Non Organik" {{ old('jenis_sampah', $laporan->jenis_sampah) == 'Non Organik' ? 'selected' : '' }}>Non Organik</option>
                            <option value="Bahaya" {{ old('jenis_sampah', $laporan->jenis_sampah) == 'Bahaya' ? 'selected' : '' }}>Bahaya</option>
                        </select>
                        @error('jenis_sampah')
                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        @enderror
